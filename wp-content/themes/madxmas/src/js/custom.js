...
jQuery(document).ready(function ($) {
  ...
  const element = document.querySelector('.ornament-left');
  const observer = new IntersectionObserver(entries => {
  element.classList.toggle( 'slide-down-animation', entries[0].isIntersecting );
  });

  observer.observe( element );

  const element_v2 = document.querySelector('.ornament-right');
  const observer_v2 = new IntersectionObserver(entries => {
  element_v2.classList.toggle( 'slide-down-animation_v2', entries[0].isIntersecting );
  });

  observer_v2.observe( element_v2 );
});
